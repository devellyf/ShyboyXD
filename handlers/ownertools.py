...
async def botstats(_, message: Message):
    ...
    await message.reply_text(
        text=f"📊 Sᴛᴀᴛs Oғ @{BOT_USERNAME} \n\n🤖 Bᴏᴛ Vᴇʀsɪᴏɴ: `{__version__}` \n\n🙎🏼 Tᴏᴛᴀʟ Usᴇʀs: \n » Oɴ Pᴍ: `{total_users}` \n\n💾 Dɪsᴋ Sᴛᴏʀᴀɢᴇ: \n » Dɪsᴋ Sᴘᴀᴄᴇ: `{total}` \n » Usᴀɢᴇ: `{used}({disk_usage}%)` \n » Fʀᴇᴇ: `{free}` \n\n🎛 Hᴀʀᴅᴡᴀʀᴇ Usᴀɢᴇ: \n » Cᴘᴜ Usᴀɢᴇ: `{cpu_usage}%` \n » Rᴀᴍ Usᴀɢᴇ: `{ram_usage}%`",
        parse_mode="Markdown",
        quote=True,
    )
    ...
    try:
        ...
        try:
            await c.send_message(
                user_id,
                f"😆 Sorry, You're Banned By Owner!** \n\nReason: `{ban_reason}` \nDuration: `{ban_duration}` day(s). \n\n**💬 Message From Owner: Ask In @Shizuka_Vc_Support If You Think This Was An Mistake.",
            )
            ban_log_text += "\n\n✅ "
        except:
            traceback.print_exc()
